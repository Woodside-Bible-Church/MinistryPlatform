...
// Upload files to a purchase request
export async function POST(
  ...
  try {
    ...
    // Get Page_ID for Project_Budget_Purchase_Requests table
    const pageResponse = await fetch(
      `${baseUrl}/tables/dp_Pages?$filter=Table_Name='Project_Budget_Purchase_Requests'&$select=Page_ID`,
      {
        headers: {
    ...
    const uploadPromises = files.map(async (file) => {
      ...
      fileFormData.append("RecordId", requestId);
    ...
    console.error("Error uploading purchase request files:", error);
    return NextResponse.json(
...
export async function GET(
  ...
  try {
    ...
    const filesUrl = `${baseUrl}/tables/dp_Files?$filter=Record_ID=${requestId} AND Page_ID=(SELECT Page_ID FROM dp_Pages WHERE Table_Name='Project_Budget_Purchase_Requests')&$select=File_ID,File_Name,File_Size,Unique_File_ID,Description,Last_Updated`;
    ...
    console.error("Error fetching purchase request files:", error);
    return NextResponse.json(
      { error: "Failed to fetch files" },
      { status: 500 }
    );
