-- Register api_Custom_GetTransactionDetails_JSON in MinistryPlatform
-- Get detailed information for a single transaction including line item, category, project, and purchase request info

-- Check if it already exists
IF NOT EXISTS (SELECT 1 FROM dp_API_Procedures WHERE Procedure_Name = 'api_Custom_GetTransactionDetails_JSON')
BEGIN
    INSERT INTO dp_API_Procedures (Procedure_Name, Description)
    VALUES ('api_Custom_GetTransactionDetails_JSON', 'Get detailed information for a single transaction');
...
WHERE Procedure_Name = 'api_Custom_GetTransactionDetails_JSON';
...
WHERE ap.Procedure_Name = 'api_Custom_GetTransactionDetails_JSON';
