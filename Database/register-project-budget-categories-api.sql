-- Register api_Custom_GetProjectBudgetCategories_JSON in MinistryPlatform
-- Get budget categories and line items for a project (no transactions)

-- Check if it already exists
IF NOT EXISTS (SELECT 1 FROM dp_API_Procedures WHERE Procedure_Name = 'api_Custom_GetProjectBudgetCategories_JSON')
BEGIN
    INSERT INTO dp_API_Procedures (Procedure_Name, Description)
    VALUES ('api_Custom_GetProjectBudgetCategories_JSON', 'Get project budget categories and line items (optimized)');
...
WHERE Procedure_Name = 'api_Custom_GetProjectBudgetCategories_JSON';
...
WHERE ap.Procedure_Name = 'api_Custom_GetProjectBudgetCategories_JSON';
