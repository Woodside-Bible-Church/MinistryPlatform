-- Register api_Custom_GetProjectBudgetSummary_JSON in MinistryPlatform
-- Lightweight procedure for project budget summary (totals only, no line items or transactions)

-- Check if it already exists
IF NOT EXISTS (SELECT 1 FROM dp_API_Procedures WHERE Procedure_Name = 'api_Custom_GetProjectBudgetSummary_JSON')
BEGIN
    INSERT INTO dp_API_Procedures (Procedure_Name, Description)
    VALUES ('api_Custom_GetProjectBudgetSummary_JSON', 'Get project budget summary with totals only (optimized)');
...
WHERE Procedure_Name = 'api_Custom_GetProjectBudgetSummary_JSON';
...
WHERE ap.Procedure_Name = 'api_Custom_GetProjectBudgetSummary_JSON';
