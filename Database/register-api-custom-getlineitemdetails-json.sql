-- Register api_Custom_GetLineItemDetails_JSON in MinistryPlatform
-- Get detailed information for a single line item including purchase requests and transactions

-- Check if it already exists
IF NOT EXISTS (SELECT 1 FROM dp_API_Procedures WHERE Procedure_Name = 'api_Custom_GetLineItemDetails_JSON')
BEGIN
    INSERT INTO dp_API_Procedures (Procedure_Name, Description)
    VALUES ('api_Custom_GetLineItemDetails_JSON', 'Get detailed information for a single line item including purchase requests and transactions');
...
WHERE ap.Procedure_Name = 'api_Custom_GetLineItemDetails_JSON';
